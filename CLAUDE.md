...
7. Every payment or credit write must be idempotent. Grant credits only through `grantTokenCredit` with a stable `dedupeId` (Stripe payment intent or event id, or the auto-reload event key), enforced by the `usage_ledger.dedupe_id` unique index. Verify the Stripe webhook signature against the raw body before reading any metadata.
